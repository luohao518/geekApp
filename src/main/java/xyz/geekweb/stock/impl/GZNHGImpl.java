 ...
 * 国债逆回购
 */
public class GZNHGImpl implements FinanceData<RealTimeDataPOJO> {
    ...
    public void initData(List<RealTimeDataPOJO> data) {
        ...
        this.data = data.stream().filter(item -> item.getFullCode().startsWith("sh204") && item.getNow() >= MIN_REVERSE_BONDS_VALUE).collect(toList());
