...
public final class JSONScanner extends JSONLexerBase {
    ...
    public boolean scanISO8601DateIfMatch(boolean strict) {
        ...
        if (rest == 8 || rest == 14 || rest == 17) {
            ...
            if (rest != 8) {
                ...
                if (rest == 17) {
                    ...
                    millis = JSONLexerBase.digits[S0] * 100 + JSONLexerBase.digits[S1] * 10 + JSONLexerBase.digits[S2];
                } else {
                    millis = 0;
                }

                hour = JSONLexerBase.digits[h0] * 10 + JSONLexerBase.digits[h1];
                minute = JSONLexerBase.digits[m0] * 10 + JSONLexerBase.digits[m1];
                seconds = JSONLexerBase.digits[s0] * 10 + JSONLexerBase.digits[s1];
            } else {
                hour = 0;
                minute = 0;
                seconds = 0;
        ...
        int hour = JSONLexerBase.digits[h0] * 10 + JSONLexerBase.digits[h1];
        int minute = JSONLexerBase.digits[m0] * 10 + JSONLexerBase.digits[m1];
        int seconds = JSONLexerBase.digits[s0] * 10 + JSONLexerBase.digits[s1];
        calendar.set(Calendar.HOUR_OF_DAY, hour);
        calendar.set(Calendar.MINUTE, minute);
        calendar.set(Calendar.SECOND, seconds);
        ...
        int millis = JSONLexerBase.digits[S0];
        int millisLen = 1;
        
        {
            char S1 = charAt(bp + 21);
            if (S1 >= '0' && S1 <= '9') {
                millis = millis * 10 + JSONLexerBase.digits[S1];
                millisLen = 2;
            }
        }
        ...
        if (millisLen == 2) {
            ...
            if (S2 >= '0' && S2 <= '9') {
                millis = millis * 10 + JSONLexerBase.digits[S2];
                millisLen = 3;
            }
        }
        ...
        if (timeZoneFlag == '+' || timeZoneFlag == '-') {
           ...
           int timeZoneOffset = (JSONLexerBase.digits[t0] * 10 + JSONLexerBase.digits[t1]) * 3600 * 1000;
           if (timeZoneFlag == '-') {
    ...
    private void setCalendar(char y0, char y1, char y2, char y3, char M0, char M1, char d0, char d1) {
        Locale local = Locale.getDefault();
        calendar = Calendar.getInstance(TimeZone.getDefault(), local);
        int year = JSONLexerBase.digits[y0] * 1000 + JSONLexerBase.digits[y1] * 100 + JSONLexerBase.digits[y2] * 10 + JSONLexerBase.digits[y3];
        int month = JSONLexerBase.digits[M0] * 10 + JSONLexerBase.digits[M1] - 1;
        int day = JSONLexerBase.digits[d0] * 10 + JSONLexerBase.digits[d1];
    ...
    public int scanFieldInt(char[] fieldName) {
        ...
        int value;
        if (ch >= '0' && ch <= '9') {
            value = JSONLexerBase.digits[ch];
            for (;;) {
                ch = charAt(index++);
                if (ch >= '0' && ch <= '9') {
                    value = value * 10 + JSONLexerBase.digits[ch];
                } else if (ch == '.') {
                    matchStat = NOT_MATCH;
                    return 0;
                } else {
